...
class BaseClient(object):
    ...
    def _request(self, func, url, *args, **kwargs):
        ...
        try:
            ...
            logging.info(e.response.content)
...
class Public(BaseClient):
    ...
    def ticker(self):
        ...
        """
        return self._get("v1/pubticker/{symbol}".format(symbol=self.symbol), return_json=True)
    ...
    def trades(self):
        return self._get("v1/trades/{symbol}".format(symbol=self.symbol), return_json=True)

    def orders(self):
        return self._get("v1/book/{symbol}?group=1&limit_bids=100&limit_asks=100".format(symbol=self.symbol), return_json=True)


class Trading(Public):

    def __init__(self, symbol, key, secret, *args, **kwargs):
        """
        Stores the username, key, and secret which is used when making POST
        requests to Bitfinex.
        """
        super(Trading, self).__init__(symbol=symbol,
    ...
    def _get_nonce(self):
        ...
        self._nonce = max(int(time.time()*1000), nonce)
        return self._nonce
    ...
    def _post(self, *args, **kwargs):
        ...
        sig = hmac.new(bytes(secret, 'utf-8'), payload, hashlib.sha384)
        sig = sig.hexdigest()
    ...
    def cancel_order(self, order_id):
        ...
        return self._post("/v1/order/cancel", data=data, return_json=True)
    ...
    def status(self, order_id):
        ...
        return self._post("/v1/order/status", data=data, return_json=True)
